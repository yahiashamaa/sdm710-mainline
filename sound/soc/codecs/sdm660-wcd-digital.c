...
struct sdm660_wcd_digital_priv {
...
static const struct snd_kcontrol_new sdm660_wcd_digital_snd_controls[] = {
...
static int sdm660_wcd_digital_enable_interpolator(
...
static int sdm660_wcd_digital_enable_dec(struct snd_soc_dapm_widget *w,
					  struct snd_kcontrol *kcontrol,
					  int event)
{
	struct snd_soc_component *component = snd_soc_dapm_to_component(w->dapm);
...
static int sdm660_wcd_digital_enable_dmic(struct snd_soc_dapm_widget *w,
					   struct snd_kcontrol *kcontrol,
					   int event)
{
	struct snd_soc_component *component = snd_soc_dapm_to_component(w->dapm);
...
static const struct snd_soc_dapm_widget sdm660_wcd_digital_dapm_widgets[] = {
	/*RX stuff */
	...
	SND_SOC_DAPM_MIXER_E("RX1 INT", LPASS_CDC_CLK_RX_B1_CTL, 0, 0, NULL,
			     0, sdm660_wcd_digital_enable_interpolator,
	...
	SND_SOC_DAPM_ADC_E("DMIC1", NULL, SND_SOC_NOPM, 0, 0,
			   sdm660_wcd_digital_enable_dmic,
...
static int sdm660_wcd_digital_get_clks(struct platform_device *pdev,
					struct sdm660_wcd_digital_priv	*priv)
{
...
static int sdm660_wcd_digital_component_probe(struct snd_soc_component *component)
{
	struct sdm660_wcd_digital_priv *priv = dev_get_drvdata(component->dev);
...
static int sdm660_wcd_digital_component_set_sysclk(struct snd_soc_component *component,
						int clk_id, int source,
						unsigned int freq, int dir)
{
	struct sdm660_wcd_digital_priv *p = dev_get_drvdata(component->dev);

	return clk_set_rate(p->mclk, freq);
}

static int sdm660_wcd_digital_hw_params(struct snd_pcm_substream *substream,
...
static const struct snd_soc_dapm_route sdm660_wcd_digital_audio_map[] = {
...
static int sdm660_wcd_digital_startup(struct snd_pcm_substream *substream,
				       struct snd_soc_dai *dai)
{
	struct snd_soc_component *component = dai->component;
	struct sdm660_wcd_digital_priv *sdm660_wcd;
	unsigned long mclk_rate;

	sdm660_wcd = snd_soc_component_get_drvdata(component);
	...
	mclk_rate = clk_get_rate(sdm660_wcd->mclk);
	switch (mclk_rate) {
	case 12288000:
...
static void sdm660_wcd_digital_shutdown(struct snd_pcm_substream *substream,
					 struct snd_soc_dai *dai)
{
...
static const struct snd_soc_dai_ops sdm660_wcd_digital_dai_ops = {
	.startup = sdm660_wcd_digital_startup,
	.shutdown = sdm660_wcd_digital_shutdown,
	.hw_params = sdm660_wcd_digital_hw_params,
};

static struct snd_soc_dai_driver sdm660_wcd_digital_dai[] = {
	[0] = {
	       .name = "sdm660_wcd_digital_i2s_rx1",
	       .id = 0,
	       .playback = {
	       ...
	       .ops = &sdm660_wcd_digital_dai_ops,
	       },
	[1] = {
	       .name = "sdm660_wcd_digital_i2s_tx1",
	       .id = 1,
	       .capture = {
...
static const struct snd_soc_component_driver sdm660_wcd_digital = {
	.probe			= sdm660_wcd_digital_component_probe,
	.set_sysclk		= sdm660_wcd_digital_component_set_sysclk,
	.controls		= sdm660_wcd_digital_snd_controls,
	.num_controls		= ARRAY_SIZE(sdm660_wcd_digital_snd_controls),
	.dapm_widgets		= sdm660_wcd_digital_dapm_widgets,
	.num_dapm_widgets	= ARRAY_SIZE(sdm660_wcd_digital_dapm_widgets),
	.dapm_routes		= sdm660_wcd_digital_audio_map,
	.num_dapm_routes	= ARRAY_SIZE(sdm660_wcd_digital_audio_map),
	.idle_bias_on		= 1,
	.use_pmdown_time	= 1,
	.endianness		= 1,
};

static const struct regmap_config sdm660_codec_regmap_config = {
	.reg_bits = 32,
	.reg_stride = 4,
	.val_bits = 32,
...
static int sdm660_wcd_digital_probe(struct platform_device *pdev)
{
	struct sdm660_wcd_digital_priv *priv;
	struct device *dev = &pdev->dev;
	void __iomem *base;
	...
	digital_map =
	    devm_regmap_init_mmio(&pdev->dev, base,
				  &sdm660_codec_regmap_config);
	if (IS_ERR(digital_map))
		return PTR_ERR(digital_map);

	ret = sdm660_wcd_digital_get_clks(pdev, priv);
	if (ret < 0)
		return ret;
	...
	ret = devm_snd_soc_register_component(dev, &sdm660_wcd_digital,
				      sdm660_wcd_digital_dai,
				      ARRAY_SIZE(sdm660_wcd_digital_dai));
	if (ret)
		goto err_mclk;
...
static void sdm660_wcd_digital_remove(struct platform_device *pdev)
{
	struct sdm660_wcd_digital_priv *priv = dev_get_drvdata(&pdev->dev);
...
static const struct of_device_id sdm660_wcd_digital_match_table[] = {
	{ .compatible = "qcom,sdm660-wcd-digital-codec" },
	{ }
};

MODULE_DEVICE_TABLE(of, sdm660_wcd_digital_match_table);

static struct platform_driver sdm660_wcd_digital_driver = {
	.driver = {
		   .name = "sdm660-wcd-digital-codec",
		   .of_match_table = sdm660_wcd_digital_match_table,
	},
	.probe = sdm660_wcd_digital_probe,
	.remove_new = sdm660_wcd_digital_remove,
};

module_platform_driver(sdm660_wcd_digital_driver);

MODULE_AUTHOR("Srinivas Kandagatla <srinivas.kandagatla@linaro.org>");
MODULE_DESCRIPTION("SDM660 WCD Digital Codec driver");
