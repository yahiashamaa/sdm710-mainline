...
#define SDM660_WCD_ANALOG_RATES (SNDRV_PCM_RATE_8000 | SNDRV_PCM_RATE_16000 |\
				 SNDRV_PCM_RATE_32000 | SNDRV_PCM_RATE_44100 |\
				 SNDRV_PCM_RATE_48000 | SNDRV_PCM_RATE_96000 |\
				 SNDRV_PCM_RATE_192000)
#define SDM660_WCD_ANALOG_FORMATS (SNDRV_PCM_FMTBIT_S16_LE |\
...
static const struct snd_kcontrol_new pm660l_wcd_analog_snd_controls[] = {
...
static void pm660l_wcd_analog_micbias_enable(struct snd_soc_component *component)
{
	struct pm660l_wcd_analog_priv *wcd = snd_soc_component_get_drvdata(component);
...
static int pm660l_wcd_analog_enable_micbias(struct snd_soc_component *component,
					    int event, unsigned int cap_mode)
{
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		pm660l_wcd_analog_micbias_enable(component);
...
static int pm660l_wcd_analog_enable_micbias_int(struct snd_soc_dapm_widget *w,
...
static int pm660l_wcd_analog_enable_micbias1(struct snd_soc_dapm_widget *w,
					     struct snd_kcontrol *kcontrol,
					     int event)
{
	struct snd_soc_component *component = snd_soc_dapm_to_component(w->dapm);
	...
	return pm660l_wcd_analog_enable_micbias(component, event,
						wcd->micbias1_cap_mode);
}

static int pm660l_wcd_analog_enable_micbias2(struct snd_soc_dapm_widget *w,
					     struct snd_kcontrol *kcontrol,
					     int event)
{
	struct snd_soc_component *component = snd_soc_dapm_to_component(w->dapm);
...
static int pm660l_mbhc_configure_bias(struct pm660l_wcd_analog_priv *priv,
...
static void pm660l_wcd_setup_mbhc(struct pm660l_wcd_analog_priv *wcd)
{
	...
	pm660l_mbhc_configure_bias(wcd, micbias_enabled);
...
static int pm660l_wcd_analog_enable_micbias_int2(struct
						  snd_soc_dapm_widget
						  ...
						  *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		break;
	case SND_SOC_DAPM_POST_PMU:
		pm660l_mbhc_configure_bias(wcd, true);
		break;
	case SND_SOC_DAPM_POST_PMD:
		pm660l_mbhc_configure_bias(wcd, false);
		break;
	}

	return pm660l_wcd_analog_enable_micbias_int(w, kcontrol, event);
}

static int pm660l_wcd_analog_enable_adc(struct snd_soc_dapm_widget *w,
					 struct snd_kcontrol *kcontrol,
					 int event)
{
	struct snd_soc_component *component = snd_soc_dapm_to_component(w->dapm);
...
static int pm660l_wcd_analog_enable_spk_pa(struct snd_soc_dapm_widget *w,
					    struct snd_kcontrol *kcontrol,
					    int event)
{
	struct snd_soc_component *component = snd_soc_dapm_to_component(w->dapm);
...
static int pm660l_wcd_analog_enable_ear_pa(struct snd_soc_dapm_widget *w,
					    struct snd_kcontrol *kcontrol,
					    int event)
{
	struct snd_soc_component *component = snd_soc_dapm_to_component(w->dapm);
...
static int pm660l_wcd_analog_enable_hphl_pa(struct snd_soc_dapm_widget *w,
					    struct snd_kcontrol *kcontrol,
					    int event)
{
	struct snd_soc_component *component = snd_soc_dapm_to_component(w->dapm);
...
static int pm660l_wcd_analog_enable_hphr_pa(struct snd_soc_dapm_widget *w,
					    struct snd_kcontrol *kcontrol,
					    int event)
{
	struct snd_soc_component *component = snd_soc_dapm_to_component(w->dapm);
...
static int pm660l_wcd_analog_probe(struct snd_soc_component *component)
{
	struct pm660l_wcd_analog_priv *priv = dev_get_drvdata(component->dev);
	int err, reg;
	...
	pm660l_wcd_setup_mbhc(priv);

	return 0;
}

static void pm660l_wcd_analog_remove(struct snd_soc_component *component)
{
	struct pm660l_wcd_analog_priv *priv = dev_get_drvdata(component->dev);
...
static const struct snd_soc_dapm_widget pm660l_wcd_analog_dapm_widgets[] = {
	...
	SND_SOC_DAPM_PGA_E("EAR PA", SND_SOC_NOPM,
			   0, 0, NULL, 0,
			   pm660l_wcd_analog_enable_ear_pa,
	...
	SND_SOC_DAPM_PGA_E("SPK PA", CDC_A_SPKR_DRV_CTL,
			   6, 0, NULL, 0,
			   pm660l_wcd_analog_enable_spk_pa,
			   SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
			   SND_SOC_DAPM_PRE_PMD | SND_SOC_DAPM_POST_PMD),
	...
	SND_SOC_DAPM_SUPPLY("MIC_BIAS1", CDC_A_MICB_1_EN, 7, 0,
			    pm660l_wcd_analog_enable_micbias1,
			    SND_SOC_DAPM_POST_PMU),
	SND_SOC_DAPM_SUPPLY("MIC_BIAS2", CDC_A_MICB_2_EN, 7, 0,
			    pm660l_wcd_analog_enable_micbias2,
			    SND_SOC_DAPM_POST_PMU),
	...
	SND_SOC_DAPM_SUPPLY("MIC BIAS Internal1", CDC_A_MICB_1_INT_RBIAS, 7, 0,
			    pm660l_wcd_analog_enable_micbias_int,
			    SND_SOC_DAPM_PRE_PMU),
	SND_SOC_DAPM_SUPPLY("MIC BIAS Internal2", CDC_A_MICB_1_INT_RBIAS, 4, 0,
			    pm660l_wcd_analog_enable_micbias_int2,
			    SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
...
static int pm660l_wcd_analog_set_jack(struct snd_soc_component *component,
				      struct snd_soc_jack *jack,
				      void *data)
{
	struct pm660l_wcd_analog_priv *wcd = snd_soc_component_get_drvdata(component);
...
{
	struct pm660l_wcd_analog_priv *priv = arg;
...
static irqreturn_t pm660l_mbhc_switch_irq_handler(int irq, void *arg)
{
	struct pm660l_wcd_analog_priv *priv = arg;
	struct snd_soc_component *component = priv->component;
	bool ins = false;
	...
	if (ins) { /* hs insertion */
		...
		pm660l_mbhc_configure_bias(priv, micbias_enabled);
...
static struct snd_soc_dai_driver pm660l_wcd_analog_dai[] = {
	[0] = {
	       .name = "pm660l_wcd_analog_pdm_rx",
	       .id = 0,
	       .playback = {
			    .stream_name = "PDM Playback",
			    .rates = SDM660_WCD_ANALOG_RATES,
			    .formats = SDM660_WCD_ANALOG_FORMATS,
			    .channels_min = 1,
			    .channels_max = 3,
			    },
	       },
	[1] = {
	       .name = "pm660l_wcd_analog_pdm_tx",
	       .id = 1,
	       .capture = {
...
static int pm660l_wcd_analog_set_sysclk(struct snd_soc_component *component,
			  int clk_id, int source, unsigned int freq, int dir)
{
	return 0;
}

static const struct snd_soc_component_driver pm660l_wcd_analog = {
	.probe			= pm660l_wcd_analog_probe,
	.remove			= pm660l_wcd_analog_remove,
	.set_sysclk		= pm660l_wcd_analog_set_sysclk,
	.set_jack		= pm660l_wcd_analog_set_jack,
	.controls		= pm660l_wcd_analog_snd_controls,
	.num_controls		= ARRAY_SIZE(pm660l_wcd_analog_snd_controls),
	.dapm_widgets		= pm660l_wcd_analog_dapm_widgets,
	.num_dapm_widgets	= ARRAY_SIZE(pm660l_wcd_analog_dapm_widgets),
	.dapm_routes		= pm660l_wcd_analog_audio_map,
	.num_dapm_routes	= ARRAY_SIZE(pm660l_wcd_analog_audio_map),
	.idle_bias_on		= 1,
	.use_pmdown_time	= 1,
	.endianness		= 1,
};

static int pm660l_wcd_analog_parse_dt(struct device *dev,
				       struct pm660l_wcd_analog_priv *priv)
{
	int rval;
...
static int pm660l_wcd_analog_spmi_probe(struct platform_device *pdev)
{
	struct pm660l_wcd_analog_priv *priv;
	...
	ret = pm660l_wcd_analog_parse_dt(dev, priv);
	if (ret < 0)
		return ret;
	...
	ret = devm_request_threaded_irq(dev, irq, NULL,
			       pm660l_mbhc_switch_irq_handler,
	...
	return devm_snd_soc_register_component(dev, &pm660l_wcd_analog,
				      pm660l_wcd_analog_dai,
...
static const struct of_device_id pm660l_wcd_analog_spmi_match_table[] = {
	{ .compatible = "qcom,pm660l-wcd-analog-codec", },
	{ }
};

MODULE_DEVICE_TABLE(of, pm660l_wcd_analog_spmi_match_table);

static struct platform_driver pm660l_wcd_analog_spmi_driver = {
	.driver = {
		   .name = "qcom,pm660l-wcd-spmi-codec",
		   .of_match_table = pm660l_wcd_analog_spmi_match_table,
	},
	.probe = pm660l_wcd_analog_spmi_probe,
	.remove_new = pm660l_wcd_analog_spmi_remove,
};

module_platform_driver(pm660l_wcd_analog_spmi_driver);

MODULE_AUTHOR("Srinivas Kandagatla <srinivas.kandagatla@linaro.org>");
MODULE_AUTHOR("Richard Acayan <mailingradian@gmail.com>");
MODULE_DESCRIPTION("PMIC PM660L WCD Analog Codec driver");
