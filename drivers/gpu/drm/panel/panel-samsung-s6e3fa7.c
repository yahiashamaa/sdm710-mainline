...
static int s6e3fa7_sdc_enable(struct drm_panel *panel)
{
	struct s6e3fa7_sdc *ctx = to_s6e3fa7_sdc(panel);
...
static int s6e3fa7_sdc_bl_get_brightness(struct backlight_device *bl)
{
	struct mipi_dsi_device *dsi = bl_get_data(bl);
	u16 brightness;
	int ret;
...
static const struct backlight_ops s6e3fa7_sdc_bl_ops = {
	...
	.get_brightness = s6e3fa7_sdc_bl_get_brightness,
};
...
static void s6e3fa7_sdc_remove(struct mipi_dsi_device *dsi)
{
