...
static void drv2624_haptics_work(struct work_struct *work)
{
	...
	mod_timer(&drv2624->timer, msecs_to_jiffies(100));
}
...
static void drv2624_haptics_stop(struct timer_list *timer)
{
	struct drv2624_data *drv2624;

	drv2624 = from_timer(drv2624, timer, timer);
	queue_work(drv2624->drv2624_wq, &drv2624->stop_work);
}
...
static int drv2624_haptics_upload(struct input_dev *dev,
				   struct ff_effect *effect,
				    ...
				    int value)
{
	...
	if (value)
		...
		schedule_work(&drv2624->stop_work);
...
static void drv2624_haptics_set_gain(struct input_dev *dev, u16 gain)
{
	...
	drv2624_reg_write(drv2624, DRV2624_REG_RTP_INPUT, gain / 512);
}
...
static int haptics_init(struct drv2624_data *drv2624)
{
	...
	input_set_capability(drv2624->input_dev, EV_FF, FF_GAIN);

	timer_setup(&drv2624->timer, drv2624_haptics_stop, 0);

	ret = input_ff_create(drv2624->input_dev, 1);
	if (ret) {
		dev_err(drv2624->dev, "Could not create input device: %d\n", ret);
		return ret;
	}

	drv2624->input_dev->ff->upload = drv2624_haptics_upload;
	drv2624->input_dev->ff->playback = drv2624_haptics_playback;
	drv2624->input_dev->ff->erase = drv2624_haptics_erase;
	drv2624->input_dev->ff->set_gain = drv2624_haptics_set_gain;
